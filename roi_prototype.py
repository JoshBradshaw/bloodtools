...
class ROISelectPlot(QtGui.QWidget):
    ...
    @QTSlotExceptionRationalizer("bool")
    def make_image(self, im, image_rois, image_filename):
        self.clear_roi()
        self.axes.clear()
        if image_filename in image_rois:
            self.image_ROI = image_rois[image_filename]
...
class MainWindow(QtGui.QWidget):
    ...
    def change_image(self):
        ...
        self.plot_im.make_image(self.images[self.image_index], self.image_ROIs, self.image_filename)
    ...
    def choose_dir(self, event):
        ...
        if out:
            ...
            image_filename = self.image_filename_list[self.image_index]
            self.plot_im.make_image(self.images[0], self.image_ROIs, image_filename)
            
            num, demon = '01', str(len(self.images)).rjust(2, '0')
            self.slice_label.setText("{}/{}".format(num, demon))
            
            # if previously saved ROIs exist for this directory, reload them
    ...
    def start_roi(self):
        ...
        if roi_scope == "All Slices":
            for img_fn in self.image_filename_list:
                self.image_ROIs[image_fn] = self.plot_im.image_ROI
        else:
            self.image_ROIs[self.image_filename] = self.plot_im.image_ROI

...
class model:
    ...
    def bootstrap(self,N=10000,**data):
        ...
        for n in xrange(N):
            ...
            if dy.size>1:
                out,ch,ch = self.fit(x[0,ind],y[ind],dy[ind])
            else:
    ...
    def contours(self,par1,par2,par1vals,par2vals,**data):
        ...
        for ii,val1 in enumerate(par1vals):
            par1.value = val1
            for jj,val2 in enumerate(par2vals):
                par2.value = val2
                if self.npars > 0:
                    self.fit(x,y,dy)
                pars = self.pars2vals()
                out[ii,jj] = (((self.call(x,*pars)-y)/dy)**2).sum()
